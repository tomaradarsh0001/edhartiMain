...
class DemandController extends Controller
{
            ...
            return DB::transaction(function () use ($request) {
                ...
                if ($oldPropertyId || $demandId) {
                    ...
                    $newDemand =  Demand::updateOrCreate(['id' => $request->id ?? 0], [
                        ...
                        'paid_amount' => 0,
                        'carried_amount' => $carriedAmount > 0 ? $carriedAmount : 0,
    ...
    public function applicantPayForDemand($demandId)
    {
        ...
        if (in_array(getServiceCodeById($demand->status), ["DEM_PART_PAID", "DEM_PENDING"])) {
            $user = Auth::user();
            $applicantDetails = ApplicantUserDetail::where('user_id', $user->id)->first();
            $countries = Country::all();
            $states = DB::table('states')->where('country_id', 101)->get();
            return view('demand.applicant.payment-form', ['demand' => $demand, 'countries' => $countries, 'states' => $states]);
        } else {
